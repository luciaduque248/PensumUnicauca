...
import { normalizeSubjectGradeRecord } from "../data/defaultGrades";
import {
...
import type { CurriculumSection, SubjectStatus } from "../types/curriculum";
import type {
  GradeActivity,
  GradeCutId,
  StudentGradeRecords,
...
const CUT_LABELS: Record<GradeCutId, string> = {
  first: "Corte 1",
  second: "Corte 2",
  third: "Corte 3",
};

const CUT_IDS: GradeCutId[] = ["first", "second", "third"];
...
const formatGrade = (value: number | null, decimals: 1 | 2) =>
...
const getCutPercentageTotal = (activities: GradeActivity[]) =>
  roundPercentage(
...
const getCutCalculatedGrade = (
  calculation: ReturnType<typeof calculateSubjectGrade>,
  cutId: GradeCutId,
) => {
  if (cutId === "first") return calculation.firstCutGrade;
  if (cutId === "second") return calculation.secondCutGrade;
  return calculation.thirdCutGrade;
};

const createActivityId = (subjectCode: string, cutId: GradeCutId) =>
  `${subjectCode}-${cutId}-${Date.now()}-${Math.random().toString(36).slice(2, 8)}`;
...
const getUniqueActivityName = (
  ...
  const existingNames = new Set(
    currentActivities.map((activity) => normalizeText(activity.name)),
  );

  if (!existingNames.has(normalizeText(cleanName))) return cleanName;

  let suffix = 2;
  while (existingNames.has(normalizeText(`${cleanName} ${suffix}`))) {
    suffix += 1;
  }
...
function GradesPage({
  ...
  const enrolledSubjects = useMemo<EnrolledSubject[]>(() => {
    const confirmedScheduleCodes = new Set<string>();
    const confirmedScheduleNames = new Set<string>();

    if (isScheduleConfirmed) {
      scheduleClasses.forEach((scheduleClass) => {
        const normalizedCode = normalizeCode(scheduleClass.subjectCode);
        if (normalizedCode !== "") confirmedScheduleCodes.add(normalizedCode);

        const normalizedName = normalizeText(scheduleClass.subjectName);
        if (normalizedName !== "") confirmedScheduleNames.add(normalizedName);
      });
    }
    ...
    return curriculum
      .flatMap((section) =>
        section.subjects.map((subject) => {
          const isInProgress = subjectStatuses[subject.code] === "in-progress";
          const isInConfirmedSchedule =
            isScheduleConfirmed &&
            (confirmedScheduleCodes.has(normalizeCode(subject.code)) ||
              confirmedScheduleNames.has(normalizeText(subject.name)));
      ...
      .filter(
        (subject) => subject.isInProgress || subject.isInConfirmedSchedule,
      )
      .sort((firstSubject, secondSubject) => {
        const firstSemester = firstSubject.semester ?? 99;
        const secondSemester = secondSubject.semester ?? 99;
  ...
  }, [curriculum, isScheduleConfirmed, scheduleClasses, subjectStatuses]);

  const subjectRows = enrolledSubjects.map((subject) => {
    const record = normalizeSubjectGradeRecord(
      subject.code,
    ...
    return {
      subject,
      record,
      calculation: calculateSubjectGrade(record),
    };
  });

  const subjectsWithGrades = subjectRows.filter(({ record }) =>
    hasRegisteredGrades(record),
  ).length;

  const completedSubjectRows = subjectRows.filter(
    ({ calculation }) =>
      calculation.isComplete && calculation.officialOneDecimal !== null,
  );

  const completedSubjects = completedSubjectRows.length;
  const subjectsAtOrAboveThree = completedSubjectRows.filter(
    ({ calculation }) => (calculation.officialOneDecimal ?? 0) >= 3,
  ).length;

  const completedCredits = completedSubjectRows.reduce(
      ...
      : roundGradeToOfficialTenth(
          completedSubjectRows.reduce(
            (total, { subject, calculation }) =>
              total + (calculation.officialOneDecimal ?? 0) * subject.credits,
            0,
          ) / completedCredits,
        );
  ...
  const handleActivityGradeChange = (
    ...
    const parsedGrade = rawValue === "" ? null : Number(rawValue);

    if (
      parsedGrade !== null &&
      (!Number.isFinite(parsedGrade) || parsedGrade < 0 || parsedGrade > 5)
    ) {
      return;
    }

    const updatedActivities = currentRecord.cuts[cutId].activities.map(
      (activity) =>
    ...
    saveRecord(subjectCode, {
      ...currentRecord,
      cuts: {
        ...currentRecord.cuts,
        [cutId]: { activities: updatedActivities },
      },
    });
  };
  ...
  const handleActivityPercentageChange = (
    ...
    if (!primaryActivity || targetIndex < 0) return;
    ...
    if (targetIndex === 0) {
      ...
      saveRecord(subjectCode, {
        ...
        cuts: {
          ...currentRecord.cuts,
          [cutId]: { activities: currentActivities },
        },
      });
      return;
    }
    ...
    const maximumAllowed = targetActivity.percentage + primaryActivity.percentage;
    if (parsedPercentage > maximumAllowed) return;

    const difference = parsedPercentage - targetActivity.percentage;
    const updatedPrimaryPercentage = roundPercentage(
      primaryActivity.percentage - difference,
    );
    if (updatedPrimaryPercentage < 0) return;

    currentActivities[0] = {
      ...primaryActivity,
      percentage: updatedPrimaryPercentage,
    };
  ...
  const handleAddActivity = async (
    ...
    const activityTypeOptions = ACTIVITY_TYPE_OPTIONS.map(
      (activityType) =>
        `<option value="${escapeHtml(activityType)}">${escapeHtml(activityType)}</option>`,
    ).join("");

    const result = await Swal.fire<NewGradeActivityData>({
      title: `Agregar ítem al ${CUT_LABELS[cutId]}`,
      html: `
        <div class="grades-activity-modal">
          <p class="grades-activity-modal__subject">${escapeHtml(subjectName)}</p>

          <label class="grades-activity-modal__field" for="grades-activity-type">
            <span>Tipo de actividad</span>
            <select id="grades-activity-type">${activityTypeOptions}</select>
          </label>

          <label class="grades-activity-modal__field" for="grades-activity-name">
            <span>Nombre personalizado <small>(opcional)</small></span>
            <input id="grades-activity-name" type="text" maxlength="45" placeholder="Ejemplo: Quiz de Fourier" />
          </label>
      ...
      confirmButtonText: "Agregar ítem",
      ...
      preConfirm: () => {
        ...
        const selectedType = typeElement?.value.trim() ?? "";
        const customName = nameElement?.value.trim() ?? "";
        const percentage = Number(percentageElement?.value ?? "");

        if (selectedType === "") {
          Swal.showValidationMessage("Selecciona el tipo de actividad.");
          return false;
        }
        if (selectedType === "Otro" && customName === "") {
          Swal.showValidationMessage("Escribe el nombre de la actividad.");
          return false;
        }
        if (
      ...
      index === 0
        ? {
            ...
            percentage: roundPercentage(
              activity.percentage - amountTakenFromPrimary,
            ),
          }
        : activity,
    );
    ...
    updatedActivities.push({
      id: createActivityId(subjectCode, cutId),
      name: getUniqueActivityName(result.value.name, currentActivities),
  ...
  const handleDeleteActivity = async (
    ...
    const activityToDelete = currentActivities[activityIndex];
      ...
      .map((activity, index) =>
        index === 0
          ? {
              ...activity,
              percentage: roundPercentage(
                primaryActivity.percentage + activityToDelete.percentage,
              ),
            }
          : activity,
      );
    ...
    <div className="grades-page">
      <style>{`
        @media (max-width: 620px) {
          ...
          .grades-table th.grades-table__semester-column,
          .grades-table th.grades-table__code-column,
          .grades-table th.grades-table__subject-column {
            left: auto;
          }

          .grades-table td.grades-table__semester-cell,
          .grades-table td.grades-table__code-cell,
          .grades-table td.grades-table__subject-cell {
            position: static;
            left: auto;
          }
        }
      ...
      <header className="grades-header">
        <div className="grades-header__content">
          <div className="grades-header__intro">
            <p className="grades-header__eyebrow">Seguimiento de calificaciones</p>
            <div className="grades-header__title">
              <LuCalculator aria-hidden="true" />
      ...
      <main className="grades-main">
        <section className="grades-information">
          <span className="grades-information__icon" aria-hidden="true">
            <LuCircleAlert />
          </span>
          <div>
        ...
        <section className="grades-summary" aria-label="Resumen de notas">
          <article className="grades-summary-card">
            <span>Materias matriculadas</span>
            <strong>{enrolledSubjects.length}</strong>
          </article>
          <article className="grades-summary-card">
            <span>Con notas registradas</span>
            <strong>{subjectsWithGrades}</strong>
          </article>
          <article className="grades-summary-card">
            <span>Cálculo completo</span>
            <strong>{completedSubjects}</strong>
          </article>
          <article className="grades-summary-card">
            <span>Resultado de 3.0 o más</span>
            <strong>{subjectsAtOrAboveThree}</strong>
          </article>
          <article className="grades-summary-card">
            <span>Promedio del semestre</span>
            <strong>{formatGrade(semesterAverage, 1)}</strong>
          </article>
        </section>
        ...
        {subjectRows.length === 0 ? (
          <section className="grades-empty">
            <LuCalculator aria-hidden="true" />
            <h2>No hay materias matriculadas para mostrar</h2>
            <p>
          ...
          <section className="grades-sheet-section">
            ...
            <div className="grades-sheet-wrapper">
              <table className="grades-table">
                <thead>
                  <tr>
                    <th className="grades-table__semester-column">Sem.</th>
                    ...
                    <th className="grades-table__result-column">
                      Aproximado<small>1 decimal</small>
                    </th>
                    <th className="grades-table__status-column">Estado</th>
                  </tr>
                </thead>
                  ...
                  {subjectRows.map(({ subject, record, calculation }) => {
                    ...
                    const resultStatus = !hasGrades
                      ...
                      : !calculation.isComplete
                        ? "En seguimiento"
                        : (calculation.officialOneDecimal ?? 0) >= 3
                          ? "Resultado ≥ 3.0"
                          : "Resultado < 3.0";
                    ...
                    return (
                      <tr key={subject.code}>
                        ...
                        <td className="grades-table__code-cell">{subject.code}</td>
                        <td className="grades-table__subject-cell">
                          <strong>{subject.name}</strong>
                          <small>{subject.sectionTitle}</small>
                        </td>

                        {CUT_IDS.map((cutId) => {
                          const activities = record.cuts[cutId].activities;
                        ...
                        <td className="grades-table__calculated-cell">
                          {formatGrade(calculation.accumulatedTwoDecimals, 2)}
                        </td>
                        <td className="grades-table__official-cell">
                          {formatGrade(calculation.officialOneDecimal, 1)}
                        </td>
                        <td className="grades-table__status-cell">
                          <span
                            className={`grades-table__status ${
                              calculation.isComplete
                                ? (calculation.officialOneDecimal ?? 0) >= 3
